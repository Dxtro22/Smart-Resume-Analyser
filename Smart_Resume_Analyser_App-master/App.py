...
# Override ResumeParser to use our model
...
class CustomResumeParser(ResumeParser):
        ...
        if ext == 'pdf':
            try:
                with open(self.__resume, 'rb') as pdf_file:
                    ...
                    self.__details['no_of_pages'] = len(pdf_reader.pages)
    ...
    def __get_basic_details(self):
        ...
        try:
            ...
            self.__details['company_names'] = entities['Companies worked at']
        except KeyError:
            pass
...
def fetch_yt_video(link):
    ...
    try:
        if "youtu.be" in link:
            ...
            video_id = link  # Assume the link is already a video ID
        ...
        st.warning(f"Could not load video. Using direct link instead.")
        st.markdown(f"[Click here to watch the video]({link})", unsafe_allow_html=True)
        return "Video link provided"


# Update the resume_videos and interview_videos lists with valid YouTube URLs
resume_videos = [
...
def get_table_download_link(df, filename, text):
    ...
    in:  dataframe
    out: href string
    provides a Base64-encoded CSV download link
    """
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'smit' and ad_password == 'smit123':
                st.success("Welcome Admin")
                cursor.execute('''SELECT * FROM user_data''')
                data = cursor.fetchall()
                st.header("**User's👨‍💻 Data**")
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email_ID', 'Resume_Score', 'Timestamp', 'Total_Page',
                ...
                fig = px.pie(values=field_counts.values, names=field_counts.index,
                             title='Predicted Fields Based on Skills')
                st.plotly_chart(fig)
